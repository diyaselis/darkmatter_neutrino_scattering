...
fig = plt.figure()
ax = plt.axes(projection="3d")
img  = ax.scatter3D(X, Y, Z, c=10**np.log10(TS), cmap=matplotlib.colormaps['GnBu'], alpha=0.7, marker='.')
ax.set_xlabel(r'$m_\phi$ (GeV)')
ax.set_ylabel(r'$m_\chi$ (GeV)')
ax.set_zlabel('g (GeV)')
fig.colorbar(img,pad=8)
plt.tight_layout()
plt.show()

# # DOES NOT WORK!
# import plotly.graph_objects as go
# fig = go.Figure(data=[go.Surface(x=X, y=Y, z=Z, surfacecolor=TS)])
# fig.show()
