...
diff_perbin = np.abs(h_null - h_DM)
# plt.plot(center, diff_perbin,'-o')
plt.step(center, diff_perbin, where='mid')
plt.xlim(1e3, 1e7)
plt.xlabel('Energy (GeV)')
...
diff_perbin = np.abs(counts - (astro+atm+muon))
# plt.plot(center, diff_perbin,'-o')
plt.step(center, diff_perbin, where='mid')
plt.xlabel('dec')
plt.ylabel(r'$|\Delta N_{events}|$')
